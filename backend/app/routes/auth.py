...
def signup(request: SignupRequest, req: Request, db: Session = Depends(get_db)):
    ...
    is_valid, password_error = validate_password_strength(request.password)
    if not is_valid:
    ...
    # Log signup
    AuditService.log_signup(db, user.id, user.email, req)
    
    # Notify all admins
    NotificationService.notify_admins_new_signup(db, user.id, user.email, user.full_name, approval_request.id)
